...
public class DBOrder implements OrderRepository {
        ...
        while (rs.next()) {
          ...
          for (Tree.Type t : Tree.Type.values()) {
            if (t.name().equals(typeName)) {
              tmpMaterial =
                  new Tree(materialName, length, materialPrice, materialUsage, t, materialUnit);
  ...
  public void assignOrder(int ordrenummer, int userId) throws OrderNotFound {
    try (Connection conn = database.getConnection()) {
      try (PreparedStatement ps =
          conn.prepareStatement("UPDATE orders SET orders.employee_id=? WHERE orders.id=?;")) {
  ...
  public void releaseOrder(int orderId) throws OrderNotFound {
    try (Connection conn = database.getConnection()) {
      try (PreparedStatement ps =
          conn.prepareStatement("UPDATE orders SET orders.employee_id=NULL WHERE orders.id=?;")) {
    ...
    try (Connection conn = database.getConnection()) {
      ...
      try (PreparedStatement ps =
          conn.prepareStatement(
              "SELECT id FROM `usage` WHERE name=? AND material_id=? AND type_id=?")) {
  ...
  public Order createNewOrder(Order order) throws OrderException {
    ...
    String carportSql =
        "INSERT INTO carports(length, width, roof, price, partlist_id) " + "VALUE (?,?,?,?,?);";

    if (carport.hasShed()) {
      shedId = createShedInDb(carport.getShed());
      carportSql =
          "INSERT INTO carports(length, width, roof, shed_id, price, partlist_id) "
              + "VALUE (?,?,?,?,?,?);";
    }
    ...
    try (Connection conn = database.getConnection()) {
      ...
      try (PreparedStatement ps =
          conn.prepareStatement(carportSql, Statement.RETURN_GENERATED_KEYS)) {
      ...
      try (PreparedStatement ps =
          conn.prepareStatement(
              "INSERT INTO orders(width, length, customer_id, status, carport_id, margin, uuid) "
                  + "VALUE (?,?,?,?,?,?,?);",
